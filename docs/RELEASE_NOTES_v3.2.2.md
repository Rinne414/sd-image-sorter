...
## Python 3.13 support (Linux source-install fix)

A Linux user reported `pip install` failing on `numpy==1.26.4` with a `metadata-generation-failed` mesonpy source-build error. Root cause: numpy 1.26.4 (the version we locked for the v3.2.x line) only ships wheels for Python 3.9-3.12 — there is no cp313 wheel, so any system whose default `python3` is 3.13+ (Arch, Fedora 41+, Ubuntu 25.04, Debian 13, …) tries to source-build numpy and fails because most minimal Linux installs don't have meson/ninja/BLAS dev libraries.

v3.2.2 fixes this by switching to a **universal lockfile**:

- `backend/requirements-core.txt` and `backend/requirements.txt` are now generated by `uv pip compile --universal`. A single file holds both Python branches via `python_full_version` markers.
- Python 3.12 stays on the proven stack: numpy 1.26.4, onnxruntime 1.20.1 (Windows) / 1.25.0 (Linux) / 1.19.2 (macOS).
- Python 3.13 gets numpy 2.4.6 and onnxruntime 1.26.0 — the newest binaries with cp313 wheels for every supported platform.
- `repair_torch_runtime.py` now exposes `_numpy_sam3_constraint()` which returns `numpy<2.0` on Python 3.12 and `numpy>=2.1.0,<3.0` on Python 3.13, so the CUDA-torch repair installs a numpy that the rest of the binary ABI agrees with.
- Verified: full backend pytest passes 1526 / 6 skipped / 0 failed on Python 3.12.7 + numpy 1.26.4 (98 seconds), and 1526 / 6 skipped / 0 failed on Python 3.13.13 + numpy 2.4.6 + torch 2.11.0+cpu + transformers 5.6.2 (561 seconds; cold-cache CPU torch).

The Windows portable bundle still ships Python 3.12.8 — bundling 3.13 would change the download size and we want a no-surprise upgrade path. Source / Linux users can use either Python.
