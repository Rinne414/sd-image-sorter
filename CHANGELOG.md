...
v3.5.0 落地重设计路线的前两阶段：全局配色收束到全新的 `frontend/css/tokens.css`（v4.0「清爽极光」设计语言——蓝=下一步、粉=用户决定、紫=AI 产物），并新增任务入口页（启动时展示四条任务动线 + ★5 作品门面，可在设置里跳过）。技术债清扫同批落地：超大库批量删除/移除/导出升级为可取消的后台任务、AI 任务队列跨重启持久化、Linux NVIDIA 用户修复 GPU 打标，并完成 main.py/sorting/image_service 的模块拆分收尾与依赖安全清理。

v3.5.0 lands the first two phases of the redesign track: the global palette is now owned by the new `frontend/css/tokens.css` (v4.0 "Fresh Aurora" design language — blue = next action, pink = user decisions, purple = AI output), plus a new mission entry page at launch (four mission lanes + a daily ★5 cover, skippable in Settings). A debt sweep ships in the same batch: huge-library bulk delete/remove/export become cancellable background jobs, the AI job queue survives restarts, Linux NVIDIA users get GPU tagging, and the main.py / sorting / image_service module extraction plus dependency security cleanup are completed.
