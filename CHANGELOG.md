...
### Added / 新增
- **Linux portable bundle (Phase 1)** — new `sd-image-sorter-vX.X.X-linux-portable.tar.gz` asset that ships its own [astral-sh/python-build-standalone](https://github.com/astral-sh/python-build-standalone) cpython 3.13 (x86_64, baseline glibc 2.17). Same first-run flow as the Windows portable: extract, `chmod +x run-portable.sh`, run. Works on every modern x86_64 Linux distro including ones whose system Python is 3.14 (where heavy AI wheels are not yet available) or where Python is missing entirely. The existing `linux.tar.gz` source-install variant stays available for advanced users who already manage a Python 3.12 / 3.13 toolchain. `run.sh` auto-detects an extracted portable bundle (`./python/bin/python3` + `./run-portable.sh`) and forwards there, so users who double-click `run.sh` on the portable archive still get the bundled-Python path. New CI smoke job (`linux-portable-smoke`) builds the asset on a clean ubuntu-22.04 runner, extracts it into a fresh directory, and probes `http://127.0.0.1:8499/` after first-run dependency install. ARM64 / aarch64 (Raspberry Pi 5, ARM Linux servers) is planned for Phase 2.
  - **Linux 便携版打包（Phase 1）**：新增 `sd-image-sorter-vX.X.X-linux-portable.tar.gz` 发布 asset，内置 cpython 3.13（x86_64，对齐 glibc 2.17 基线，使用 [astral-sh/python-build-standalone](https://github.com/astral-sh/python-build-standalone)）。和 Windows 便携版一样：解压 → `chmod +x run-portable.sh` → 直接跑。任何现代 x86_64 Linux 发行版都可用，包括系统 Python 已经是 3.14（重型 AI wheel 还没追上）或根本没装 Python 的情况。原有的 `linux.tar.gz` 源码版保留给会自己管 Python 3.12 / 3.13 工具链的进阶用户。`run.sh` 会自动侦测同目录是否有 `./python/bin/python3` + `./run-portable.sh`，有就直接转发，所以用户在便携版资料夹里不小心点 run.sh 还是会走 bundled Python 路径。新增 CI smoke job（`linux-portable-smoke`）会在 ubuntu-22.04 全新 runner 上 build 这个 asset、解压到独立目录、首次安装核心依赖后 probe `http://127.0.0.1:8499/`。ARM64 / aarch64（Raspberry Pi 5、ARM Linux 服务器）规划在 Phase 2 处理。
...
- **Dataset Maker 5-step pipeline header**: a small stepper (导入 / 审计 / 整理 / 标注 / 输出) sits at the top of the Dataset Maker tab. Each pill scrolls to the matching anchor (toolbar / Audit panel / queue / Step A card / Step C card). Scoped to the Dataset Maker tab only.
  - **Dataset Maker 5 步骤管线表头**：Dataset Maker 标签页顶部加一条 stepper（导入 / 审计 / 整理 / 标注 / 输出），点 pill 滚到对应锚点。只作用在 Dataset Maker tab 内部。
