...
Large libraries also feel faster — bulk delete / remove / export run as cancelable background
jobs, thumbnails yield to active scans, the AI runtime gets priority + VRAM-aware scheduling,
and the similarity cache persists across restarts. Plus an adaptive UI scale for high-res
...
批量删除 / 移除 / 导出改为可取消的后台任务，缩略图在扫描时自动让路，AI 运行时获得优先级 +
显存感知调度，相似度缓存可跨重启保留。另含高分屏自适应缩放与一批 UI/UX 与 bug 修复。
未新增任何功能上限。
...
- **AI runtime scheduler — priority + VRAM + timeout**: the existing AI runtime guard gained fair priority ordering (interactive vs batch), a per-job VRAM estimate, and an opt-in acquire timeout, so concurrent AI jobs (tag + censor + similarity + aesthetic) stop fighting. Defaults are byte-for-byte the prior fully-serialized behavior.
  - **AI 运行调度器——优先级 + 显存 + 超时**：现有 AI 运行守卫新增公平优先级排序（交互 vs 批处理）、每任务显存估算与可选获取超时，让并发 AI 任务（打标 + 打码 + 相似度 + 美学）不再互抢；默认行为与此前完全串行化逐字节一致。
