    ...
    const SeparationConsole = {
        ...
        async suggestUpsample() {
            ...
            try {
                ...
                panel.textContent = t('Tag suggestion failed: ', '标签推荐失败：') + String(e.message || e);
            } finally {
        ...
        _renderTipo(panel, report) {
            ...
            title.textContent = proposals.length === 0
                ? t('TIPO found no in-vocabulary tags to add — the caption set already covers its ideas.',
                    'TIPO 没有找到可补充的词表内标签 — 现有 caption 已覆盖它的建议。')
                : t(`TIPO proposes ${proposals.length} tag(s) the taggers never scored — review and pick:`,
            ...
            apply.addEventListener('click', () => {
                ...
                window.App?.showToast?.(
                    t(`Added ${added} tag(s) to Common tags`, `已把 ${added} 个标签加入公共标签`),
                    'success');
            });
